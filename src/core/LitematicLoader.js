...
class LitematicLoader {
    ...
    readTagValue(data, offset, type) {
        ...
        switch (type) {
            ...
            case 4: // TAG_Long
                ...
                value = BigInt(high) * BigInt(0x100000000) + BigInt(low >>> 0);
                offset += 8;
                break;
            ...
            case 12: // TAG_Long_Array
                ...
                for (let i = 0; i < laLen; i++) {
                    ...
                    const l = (data[offset + 4] << 24) | (data[offset + 5] << 16) | (data[offset + 6] << 8) | data[offset + 7];
                    value.push(Number(BigInt(h) * BigInt(0x100000000) + BigInt(l >>> 0)));
                    offset += 8;
                }
                break;
    ...
    parseBlockState(blockState) {
        ...
        if (typeof blockState === 'string') {
            name = blockState;
        } else if (typeof blockState === 'object') {
            name = blockState.Name || blockState.name || blockState || 'minecraft:air';
            properties = blockState.Properties || blockState.properties || {};
        }
        
        // 转换Java ID到基岩版
        const converted = convertBlockId(name);
        
        const states = { ...converted.states };
        for (const key in properties) {
            states[key] = properties[key];
        }
