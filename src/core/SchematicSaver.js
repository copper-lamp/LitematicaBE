...
class SchematicSaver {
    ...
    encodeBlockStates(blocks, paletteMap, sizeX, sizeY, sizeZ) {
        ...
        // 转换为有符号long数组（Java的long是有符号的）
        const signedLongArray = longArray.map(v => {
            if (v >= 9223372036854775808n) {
                return Number(v - 18446744073709551616n);
            }
            return Number(v);
        });
    ...
    async writeLitematicFile(filePath, nbtData) {
        ...
        if (global.fs) {
            ...
            await new Promise((resolve, reject) => {
                fs.writeFile(filePath, Buffer.from(compressed), (err) => {
                    if (err) reject(err);
                    else resolve();
                });
            });
            ...
            logger.info(`[SchematicSaver] Written via File.writeTo: ${filePath}`);
        }
    }
