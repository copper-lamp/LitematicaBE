...
class MegaSchematicManager {
    ...
    async loadMegaSchematic(filePath, streamResult, onProgress, player) {
        ...
        if (player) {
            ...
            player.tell(`§7  正在使用调色板+GZip压缩存储...`);
        }
        ...
        try {
            ...
            if (player) {
                player.tell(`§a✓ 加载完成！`);
                player.tell(`§7  方块总数: §f${totalBlocks}`);
                player.tell(`§7  分块数: §f${chunkCount} (${CHUNK_SIZE}×${CHUNK_SIZE}×${CHUNK_SIZE})`);
                player.tell(`§7  压缩后: §f${sizeMB}MB §7(原${uncompressedMB}MB, ${ratio}%)`);
                player.tell(`§7  耗时: §f${elapsed}秒`);
                player.tell(`§7  格式: §f调色板+位打包+GZip压缩`);
            }
