...
class MaterialCounter {
    constructor() {
        ...
        this.blockNameMapper = bidirectionalConverter; // 使用新的双向转换器
    ...
    _resolveName(blockName) {
        if (!blockName) return '';
        try {
            const result = this.blockNameMapper.buildBlockData(blockName, {});
            const name = (result && result.name) ? result.name : blockName;
    ...
    countMaterials(projection) {
        ...
        // 支持 Mega 投影
        let blocksToCount = projection.blocks;
        if (projection.isMega && projection.schematicId && global.megaManager) {
            blocksToCount = this._getMegaBlocks(projection);
        }

        for (const block of blocksToCount) {
            ...
            if (materials.has(key)) {
                ...
                materials.set(key, {
                    ...
                    data: blockData,
                    count: 1,
                    chineseName: this._resolveName(blockName)
                });
            }
        }
    ...
    _getMegaBlocks(projection) {
        ...
        try {
            ...
            logger.error(`[MaterialCounter] Mega blocks load failed: ${e.message}`);
        }
        return blocks;
    }
